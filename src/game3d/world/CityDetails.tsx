...
export function CityDetails() {
  ...
  const showPointLights = !isMobile; // No point lights on mobile for performance
      ...
      {lampPositions.slice(0, lampCount).map((p, i) => (
        <group key={`lamp-${i}`} position={[p[0], 0, p[1]]}>
          ...
          <mesh position={[0.6, 5.8, 0]}>
            ...
            <meshStandardMaterial color="#ffffcc" emissive="#ffcc88" emissiveIntensity={showPointLights ? 0.4 : 0.1} />
          </mesh>
          {showPointLights && (
            <pointLight position={[0.6, 5.8, 0]} intensity={2} distance={15} color="#ffcc88" decay={2} />
          )}
        </group>
      ))}
